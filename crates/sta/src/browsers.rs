...
pub fn on_before_close(browser: &Browser) {
    ...
    match role {
        Some(Role::Tab(tab)) => {
            ...
            // Any CDP call still outstanding for this browser can never answer now. Without this
            // its timeout fires later and reports a failure against a tab that is already gone.
            crate::devtools_cdp::on_browser_closed(id);
